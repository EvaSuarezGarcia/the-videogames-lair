...
BATCH_SIZE = 1
PROGRESS_FILE = 'owners_progress.log'
NEXT_BATCH_FILE = 'next_owners_batch.log'
    ...
    try:
        with connection.cursor() as cursor:
            ...
            update_sql = "UPDATE all_steam_games_2 s JOIN (" \
                "SELECT steam_id, COUNT(*) AS owners FROM filtered_games_2 " \
                "WHERE steam_id IN (%s) GROUP BY steam_id) AS g " \
                "ON s.steam_id = g.steam_id " \
                "SET s.owners = g.owners"
